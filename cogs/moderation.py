...
class Moderator(commands.Cog):
    ...
    async def kick(self, ctx, member: discord.Member, *, reason: str = None):
        ...
        ROLE_LIST = [ctx.guild.get_role(SERVER_OWNER_ROLE), ctx.guild.get_role(ADMIN_ROLE), ctx.guild.get_role(MOD_ROLE)]
        ...
        if reason is not None:
            if SHOULD_CONTINUE:
                ...
                embed.add_field(name="Member Name", value="{}".format(member.mention))
                embed.add_field(name="User ID", value="{}".format(member.id))
                embed.add_field(name="Reason for Kick", value="{}".format(reason))
                await LOG_CHANNEL.send(embed=embed)
            else:
                await ctx.send("You tried to kick a fellow admin. Nice one buddy.")
        else:
    ...
    async def nickname(self, ctx, member: discord.Member, *, name: str = None):
        ...
        try:
            ...
            if SHOULD_CONTINUE:
                ...
                embed = discord.Embed(title="Nickname Change", colour=discord.Colour.orange())

                embed.set_thumbnail(url="{}".format(member.avatar_url))
                embed.set_author(name="Changed by {}".format(ctx.author.name))

                embed.add_field(name="Old Member Nickname", value="{}".format(nick))
                embed.add_field(name="User ID", value="{}".format(member.id))
                ...
                embed.add_field(name="New Member Nickname", value="{}".format(member.nick))
                await LOG_CHANNEL.send(embed=embed)
            else:
                await ctx.send("You tried to nickname a fellow admin. Nice one buddy.")
        ...
        try:
            ...
            if reason is not None:
                if SHOULD_CONTINUE:    
                    ...
                    embed.add_field(name="Member", value="{}".format(member.mention))
                    embed.add_field(name="User ID", value="{}".format(member.id))
                    embed.add_field(name="Reason", value="{}".format(reason))
                    await LOG_CHANNEL.send(embed=embed)
                    await ctx.send(f"{member.mention} was banned from the server for `{reason}`.")
                else:
                    await ctx.send("You tried to ban a fellow admin. Nice one buddy.")
            else:
    ...
    async def unban(self, ctx, member, *, reason: str = None):
        ...
        try:
            if reason is not None:
                ...
                embed.add_field(name="Reason", value="{}".format(reason))
                await LOG_CHANNEL.send(embed=embed)
                await ctx.send(f"{member} was unbanned from the server for `{reason}`.")
            else:
                await ctx.send("You need to supply a reason.")
        except Exception as e:
            await ctx.send(e)
        ...
        try:
            ...
            if reason is not None:
                if SHOULD_CONTINUE:
                    ...
                    embed = discord.Embed(title="Member Muted", colour=discord.Colour.orange())

                    embed.set_thumbnail(url="{}".format(member.avatar_url))
                    embed.set_author(name="Muted by {}".format(ctx.author.name))

                    embed.add_field(name="Member", value="{}".format(member.mention))
                    embed.add_field(name="User ID", value="{}".format(member.id))
                    embed.add_field(name="Reason", value="{}".format(reason))
                    await LOG_CHANNEL.send(embed=embed)
                    await ctx.send(f"{member.mention} was muted in the server for `{reason}`.")        
                else:
                    await ctx.send("You tried to mute a fellow admin. Nice one buddy.")
            else:
                await ctx.send("You need to supply a reason.")
        ...
        try:
            ...
            if reason is not None:
                if SHOULD_CONTINUE:
                    ...
                    embed = discord.Embed(title="Member Unmuted", colour=discord.Colour.orange())

                    embed.set_thumbnail(url="{}".format(member.avatar_url))
                    embed.set_author(name="Unmuted by {}".format(ctx.author.name))

                    embed.add_field(name="Member", value="{}".format(member.mention))
                    embed.add_field(name="User ID", value="{}".format(member.id))
                    embed.add_field(name="Reason", value="{}".format(reason))
                    await LOG_CHANNEL.send(embed=embed)
                    await ctx.send(f"{member.mention} was unmuted in the server for `{reason}`.")        
                else:
                    await ctx.send("You tried to mute a fellow admin. Nice one buddy.")
            else:
                await ctx.send("You need to supply a reason.")
