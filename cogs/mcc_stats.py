...
class MCCStats(commands.Cog):
    ...
    #     if SHOULD_CONTINUE:
    #         FORMED_URL = BASE_ENDPOINT + STATS_ENDPOINT
    #         payload = {"gamertag": gamertag}
    #         response = requests.post(FORMED_URL, payload)

    #         embed = discord.Embed(
    #             title="Stats for Spartan {}".format(gamertag), 
    #             colour=discord.Colour(0x40cf0f), 
    #             timestamp=datetime.datetime.utcfromtimestamp(datetime.datetime.now().timestamp())
    #         )

    #         embed.set_thumbnail(url=response.json()["emblem"])
    #         embed.set_footer(text=f"Requested by {ctx.author.display_name}")

    #         embed.add_field(name="Clantag", value=response.json()["clantag"], inline=True)
    #         embed.add_field(name="Playtime", value=response.json()["playtime"], inline=True)
    #         embed.add_field(name="Games Played", value=response.json()["gamesPlayed"], inline=True)
    #         embed.add_field(name="Wins", value=response.json()["wins"], inline=True)
    #         embed.add_field(name="Losses", value=response.json()["losses"], inline=True)
    #         embed.add_field(name="Win Ratio", value=response.json()["winRatio"], inline=True)
    #         embed.add_field(name="Kills", value=response.json()["kills"], inline=True)
    #         embed.add_field(name="Deaths", value=response.json()["deaths"], inline=True)
    #         embed.add_field(name="Kill/Death Ratio", value=response.json()["killDeathRatio"], inline=True)
    #         embed.add_field(name="Kills Per Game", value=response.json()["killsPerGame"], inline=True)
    #         embed.add_field(name="Deaths Per Game", value=response.json()["deathsPerGame"], inline=True)
    #         embed.add_field(name="Current Streak", value=response.json()["streak"], inline=True)
    ...
    #         embed.add_field(name="Game Variant", value=response.json()["games"][0]["gameVariant"], inline=True)
    #         embed.add_field(name="Map ID", value=response.json()["games"][0]["mapId"], inline=True)
    #         embed.add_field(name="Won?", value=response.json()["games"][0]["won"], inline=True)
    #         embed.add_field(name="Score", value=response.json()["games"][0]["score"], inline=True)
    #         embed.add_field(name="Assists", value=response.json()["games"][0]["assists"], inline=True)
    #         embed.add_field(name="Kills", value=response.json()["games"][0]["kills"], inline=True)
    #         embed.add_field(name="Deaths", value=response.json()["games"][0]["deaths"], inline=True)
    #         embed.add_field(name="Kill/Death Ratio", value=response.json()["games"][0]["killDeathRatio"], inline=True)
    #         embed.add_field(name="Headshot Rate", value=response.json()["games"][0]["headshotRate"], inline=True)
    #         embed.add_field(name="Played at...", value=response.json()["games"][0]["playedAtRecency"], inline=True)
    #         embed.add_field(name="Headshots", value=response.json()["games"][0]["headshots"], inline=True)
    #         embed.add_field(name="Medals", value=response.json()["games"][0]["medals"], inline=True)        
    #         await ctx.send(embed=embed)
